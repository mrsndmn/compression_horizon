...
HARD_BANS: list[tuple[str, str]] = [
    ...
    ("Подводя итог / Таким образом", r"\bподводя\s+итог\b|(?:^|[.!?…:;]\s+|\n\s*(?:[-*•]\s*)?|,\s*)таким\s+образом\s*,"),
    ("Длинное тире", r"—"),
    ...
    ("от X до Y (ложный диапазон)", r"\bот\s+(?-i:([а-яё]+))\s+до\s+(?-i:(?!\1\b)[а-яё]+)\b"),
...
SCANNER: dict[str, list[str | tuple[str, str]]] = {
    ...
    "Параллелизмы": ["не просто", "не только", ("это не X, это Y", r"это\s+не\s+[^.!?\n]{1,40}[,—:-]\s*это\b")],
    "Вводные-открытия": [
        "в современном мире",
        "в эпоху",
        "не секрет что",
