...
try:
    ...
    print(
        f"[ошибка] не хватает зависимостей сканера ({exc.name}).\n" "Поставьте один раз: pip install razdel pymorphy3",
        file=sys.stderr,
    )
    raise SystemExit(2)
...
def main() -> int:
    ...
    if _cyrillic_share(text) < 0.3:
        print("⚠ Текст не похож на русский: скилл и метрики рассчитаны на русский " "язык, отчёт ниже не показателен.\n")
    ...
    print(
        f"  предложений: {rep.rhythm.sentences}, средняя длина: {rep.rhythm.mean_len:.1f} "
        f"(min {rep.rhythm.min_len} / max {rep.rhythm.max_len}), CV: {rep.rhythm.cv_len}"
    )
    print(f"  многоточий: {rep.rhythm.ellipsis}, скобок: {rep.rhythm.parentheses}, " f"вопросов: {rep.rhythm.questions}")
    print()

    print("Морфология:")
    print(f"  {morph_verdict(rep.morph)}")
    print(f"  сущ.: {rep.morph.nouns}, глаг.форм: {rep.morph.verbs}, " f"номинализаций: {rep.morph.nominalizations}")
    print()
