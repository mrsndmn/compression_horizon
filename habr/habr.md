...
Хабр, привет! Меня зовут Дмитрий Тарасов, я исследователь в лаборатории FusionBrain Lab (AIRI). Нашу с коллегами свежую работу, **«Progressive Cramming: Reliable Token Compression and What It Reveals»**, недавно приняли на **ICML 2026**. Хочу рассказать, что мы обнаружили.

Начнём с почти магического факта. Год назад в замечательной работе ["Cramming 1568 Tokens into a Single Vector and Back Again"](https://arxiv.org/abs/2502.13063) Юры Куратова и его команды показали, что в **один-единственный входной эмбеддинг** замороженной LLM можно «упаковать» до **1568 токенов** текста так, что модель потом слово в слово его восстанавливает. Полторы тысячи токенов (несколько абзацев) в одном векторе размерности несколько тысяч. Звучит как невероятная плотность памяти. Кстати, разбор этой статьи уже [был на хабре](https://habr.com/ru/articles/906592/).

Этот результат нас восхитил. И насторожил. Одно дело *восстановить* текст, и совсем другое, *понимать* его. Что, если «идеальная» реконструкция на самом деле не хранит смысл? В нашей статье мы:
...
В предыдущей работе фиксировали **бюджет токенов** (например, сжимаем ровно 512, 1024, 1568 токенов) и считали успехом при точности реконструкции 99% токенов. Причём под **teacher forcing**, то есть подавая модели на каждом шаге *правильный* предыдущий токен, а не предсказанные моделью токены, которые могут быть с ошибкой.
...
Вывод: **фиксированный бюджет + порог 99%** это плохая метрика. Она измеряет усреднённую точность, а не то, будет ли текст реально восстановлен. Нам нужна процедура, которая гарантирует настоящую, 100%-ную реконструкцию и заодно честно говорит, где проходит граница возможностей.
...
Точки: оптимизированный эмбеддинг на каждой длине префикса. Цветные области: «бассейны» высокой точности реконструкции. По мере роста текста бассейн **сжимается**, и оптимизировать становится всё труднее.
...
Мы добавили в лосс реконструкции **температуру** $T$: делим логиты на $T$ перед софтмаксом. $T > 1$ «размягчает» распределение, а $T < 1$ заостряет. Тонкость в том, что критерий сходимости у нас это совпадение по **argmax**, а он инвариантен к любому положительному масштабированию логитов. Значит, температура не может изменить, *какое* решение достижимо в принципе, она меняет только **путь** оптимизатора к нему. И вот что из этого вышло:

![CE-температура: число сжатых токенов (левая ось) и information gain (правая ось) для pythia-1.4b и Llama-3.1-8B.](assets/ce_temperature_tokens_ig.png)

*Число сжатых токенов (левая синяя ось) и information gain (правая красная ось) в зависимости от температуры $T$. Число токенов образует перевёрнутую параболу с пиком при холодном $T{=}0.25$, а information gain максимален при более тёплом $T{\approx}0.75$: «сколько токенов влезло» и «сколько информации несёт эмбеддинг» оптимизируются при разных температурах. Показан сырой лосс $\mathrm{CE}(\mathbf{z}/T)$; $T^2$-компенсированный вариант совпадает с ним в пределах шума, то есть эффект идёт от формы распределения, а не от размера шага.*
