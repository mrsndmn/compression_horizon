...
![Число сжатых токенов в зависимости от температуры для pythia-1.4b и Llama-3.1-8B: перевёрнутая парабола с пиком при холодном T=0.25.](assets/ce_temperature_compressed_tokens.png)

*Число сжатых токенов vs температура $T$. У обеих моделей — перевёрнутая парабола с максимумом при холодном $T{=}0.25$. Сплошная линия — сырой лосс $\mathrm{CE}(\mathbf{z}/T)$, пунктир — $T^2$-компенсированный (конвенция Хинтона); они совпадают в пределах шума, то есть эффект идёт от формы распределения, а не от размера шага.*

![Information gain в зависимости от температуры для pythia-1.4b и Llama-3.1-8B: пик при более тёплом T≈0.75.](assets/ce_temperature_information_gain.png)

*Information gain (бит) vs температура $T$. Пик приходится на более тёплое $T{\approx}0.75$ — не туда, где максимум числа токенов. «Сколько токенов влезло» и «сколько информации несёт эмбеддинг» максимизируются при разных температурах.*
