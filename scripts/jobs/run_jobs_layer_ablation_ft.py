...
Usage:
    python scripts/jobs/run_jobs_layer_ablation_ft.py --dry
    python scripts/jobs/run_jobs_layer_ablation_ft.py
"""
...
# Full pretrained model: reference row (waited on, not retried, by a watcher).
REFERENCE_OUT_DIR = "artifacts/experiments_progressive/sl_4096_SmolLM2-1.7B_ds_pg19_1k_limit_50_lr_0.1"

# Wiring for the watcher's table-regeneration + paper-text step. The table holds
# interleaved un-finetuned / finetuned rows; these labels match its names_mapping.
...
def submit_experiment(experiment, client, extra_options, in_progress_descs=None, dry=False, force=False):
    ...
    if not os.path.isdir(ckpt):
        print(f"\033[31mMissing finetuned checkpoint, run run_jobs_finetune_truncated.py first:\033[0m {ckpt}")
        return None
...
def main():
    parser = argparse.ArgumentParser(
        description="Launch progressive cramming jobs for the finetuned transformer-depth ablation."
    )
