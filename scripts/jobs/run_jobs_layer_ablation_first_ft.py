"""Launch baseline progressive-cramming jobs for the FINETUNED first-only depth checkpoints.

Companion to run_jobs_layer_ablation_ft.py / _llama.py (the first-N+last-N arms).
After run_jobs_finetune_first.py finetunes each first-only checkpoint
(``…-first{N}-ftw``, for SmolLM2-1.7B and Meta-Llama-3.1-8B, N in {1,2,4,8}), this
launcher submits the same baseline progressive-cramming run used everywhere else in
the matrix, reusing :func:`run_jobs_progressive.render_job` so output-dir names and
payloads stay byte-identical (``embedding_init_method=random0.02``, lr 0.1,
cross_entropy, num_alignment_layers=1, no low-dim).

This is the **finetuned arm only** (the user chose finetuned-only for the first-only
ablation). The finetuned runs land in ``sl_4096_<model>-first{N}-ftw_ds_pg19_1k_limit_50_lr_0.1``.
The full-depth reference rows already exist for both models (24-layer SmolLM2 and
32-layer Llama) and are waited on (not retried) by the watcher.

``tab:layer_ablation`` is EXTENDED with these first-only rows as a deliberate
follow-up after the evals finish; the watcher only notifies, it does not regenerate
the table.

Usage:
    python scripts/jobs/run_jobs_layer_ablation_first_ft.py --dry
    python scripts/jobs/run_jobs_layer_ablation_first_ft.py
"""
...
EXPERIMENTS = [
    ...
    for ck in LAYER_ABLATION_FIRST_FT_CHECKPOINTS
]
...
# Wiring for the follow-up table step (extend tab:layer_ablation with the first-only
# rows; done by hand after evals finish, not by the watcher).
TABLE_NAME = "tab:layer_ablation"
APPENDIX_MARKER = "layer-ablation-trend"
ROW_LABELS = [
    "SmolLM2-1.7B, first 1 layer (finetuned)",
    "SmolLM2-1.7B, first 2 layers (finetuned)",
    "SmolLM2-1.7B, first 4 layers (finetuned)",
    "SmolLM2-1.7B, first 8 layers (finetuned)",
    "Llama-3.1-8B, first 1 layer (finetuned)",
...
def submit_experiment(experiment, client, extra_options, in_progress_descs=None, dry=False, force=False):
    ...
    if not os.path.isdir(ckpt):
        print(f"\033[31mMissing finetuned checkpoint, run run_jobs_finetune_first.py first:\033[0m {ckpt}")
        return None
...
def main():
    parser = argparse.ArgumentParser(
        description="Launch progressive cramming jobs for the finetuned first-only transformer-depth ablation."
    )
